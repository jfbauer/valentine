...
This folder contains everything needed to deploy **valentine** to production.
...
- `ghcr.io/appuser/valentine-php:latest`
- `ghcr.io/appuser/valentine-nginx:latest`
...
docker compose logs valentine-nginx
docker compose logs valentine-postgres
```
...
```bash
# Run migrations
docker compose exec valentine-php php artisan migrate --force
...
**Image pull failures:**
- Ensure you have access to `ghcr.io/appuser/valentine-*`
...
- Check application logs: `docker compose logs valentine-php`
- Review Laravel logs: `docker compose exec valentine-php tail -f storage/logs/laravel.log`
- Database logs: `docker compose logs valentine-postgres`